...
class Calibrator(object):
    ...
    def stereo_calibrate(self,
        ...
        print(f"开始双目标定过程...")
        print(f"参数设置:")
        print(f"- 左相机配置文件: {left_file}")
        print(f"- 右相机配置文件: {right_file}")
        print(f"- 左相机图片目录: {left_dir}")
        print(f"- 右相机图片目录: {right_dir}")
        print(f"- 左相机图片前缀: {left_prefix}")
        print(f"- 右相机图片前缀: {right_prefix}")
        ...
        ret, K1, D1, K2, D2, R, T, E, F = self.stereoCalibrate(self.points_world_xyz,
            ...
            print("平移矩阵 T:\n{}".format(T))
        print("--" * 30)
            print("双目标定误差 RMS = {}".format(ret))
        print("PS:若误差超过0.1，建议重新调整摄像头并标定")
        except Exception as e:
            print(f"双目标定失败: {str(e)}")
            return
        ...
        save_stereo_coefficients(save_file, K1, D1, K2, D2, R, T, E, F, R1, R2, P1, P2, Q, self.image_size)
            print("标定结果已保存到: {}".format(save_file))
        except Exception as e:
